...
{
    public class EnemyAerialUnit : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (/*isEnemyInAttackRange*/ Vector3.Distance(transform.position, target.transform.position) <= minAttackRange)
            {
        ...
        private void OnAttackSingleUnit(GameObject attackTarget, float damageAmount)
        {
            ...
            if (ReferenceEquals(this.gameObject, attackTarget))
            {
                ...
                else
                {
                    ...
                    EnemySpawner.Instance.aerialEnemyList.Remove(this.gameObject);
